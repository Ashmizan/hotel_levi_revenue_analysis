...
2. Customer geography and revenue
...
5. If you were the hotel's revenue manager, which room category and booking channel would you prioritize, and why?
  From Q19:
...
Recommendation: I would prioritize the Deluxe Sky Suite and the Corporate booking channel. 
  ...
  6. What is the average nightly room rate in winter compared with the other seasons?
  SELECT
...
Average_nightly_rate
  ...
  7. What is the average winter nightly rate for each room type?
    SELECT room_type,
AVG(nightly_rate_eur) AS average_nightly_rate
FROM bookings
WHERE season = 'Winter'
...
ORDER BY average_nightly_rate DESC;
  ...
  8. Which room type has the highest average winter nightly rate?
    SELECT room_type,
AVG(nightly_rate_eur) AS average_nightly_rate
FROM bookings
WHERE season = 'Winter'
...
LIMIT 1;
  ...
  9. Which room type has the lowest average winter nightly rate?
    SELECT room_type,
AVG(nightly_rate_eur) AS average_nightly_rate
FROM bookings
WHERE season = 'Winter'
...
LIMIT 1;
  ...
  10. Which room type experiences the largest increase in nightly rate during winter compared with the other seasons?
  SELECT
    room_type,
...
LIMIT 1;
  ...
  11. What is the percentage change in average nightly rate for each room type  between winter and non-winter?
      SELECT
    room_type,
...
ORDER BY percentage_change DESC;
  ...
  12. How does the Deluxe Sky Suite average nightly rate change between winter and non-winter?
    SELECT
    AVG(nightly_rate_eur) FILTER (WHERE season = 'Winter') AS winter_avg,
    AVG(nightly_rate_eur) FILTER (WHERE season != 'Winter') AS non_winter_avg,
    AVG(nightly_rate_eur) FILTER (WHERE season = 'Winter')
...
WHERE room_type = 'Deluxe Sky Suite';
  ...
  13. Does the Sky Suite experience a larger winter price increase than the Deluxe Room?
    SELECT
    room_type,
    AVG(nightly_rate_eur) FILTER (WHERE season = 'Winter') AS winter_avg,
    AVG(nightly_rate_eur) FILTER (WHERE season != 'Winter') AS non_winter_avg,
...
ORDER BY winter_increase DESC;
  ...
  14. Which room type has the largest difference between its minimum and maximum nightly rate?
    SELECT
    room_type,
...
LIMIT 1;
  ...
  15. Which room type appears to have the most variable pricing across seasons?
    SELECT
    room_type,
...
LIMIT 1;
  ...
  16. Which room type receives the most bookings during winter?
    SELECT
    room_type,
...
LIMIT 1;
  ...
  17. Which room type generates the highest total winter revenue?
SELECT
    room_type,
...
LIMIT 1;
  ...
  18. Which room type generates the highest average revenue per winter booking?
    SELECT
    room_type,
...
LIMIT 1;
  ...
  19. Which room type has the highest average number of nights per winter booking?
    SELECT
    room_type,
...
LIMIT 1;
  ...
  20. Compare the Deluxe Room, Sky Suite and Deluxe Sky Suite during winter in terms of:
  booking volume
...
ORDER BY total_revenue DESC;
  ...
  21. Which room types have high winter rates but relatively low booking demand?
    SELECT
    room_type,
    COUNT(*) AS booking_volume,
...
ORDER BY average_winter_rate DESC;
  ...
  22. Which room types have lower winter rates but relatively high demand?
    SELECT
    room_type,
    COUNT(*) AS booking_volume,
    AVG(nightly_rate_eur) AS average_winter_rate
...
ORDER BY average_winter_rate ASC;
  ...
  23. Do higher winter nightly rates appear to be associated with shorter stays?
    SELECT
    room_type,
    AVG(nightly_rate_eur) AS average_winter_rate,
    AVG(nights) AS average_nights
...
ORDER BY average_winter_rate DESC;
  ...
  24. Which booking channel generates the highest winter revenue, and does it also
...
ORDER BY winter_revenue DESC;
  ...
  25. If Hotel Levi wanted to maximize winter revenue, which room type and booking channel
